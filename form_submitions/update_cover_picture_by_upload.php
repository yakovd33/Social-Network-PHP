<?php
    ...
    if ($config->user->loggedin()) {
        if (isset($_FILES['file'])) {
            ...
            $insert_to_photos_table = $config->link->query("INSERT INTO `photos`(`id`, `user_id`, `user_photos_index`, `date`, `path`, `type`, `active`) VALUES (NULL, {$_SESSION['user_id']}, {$user_photos_index}, NOW(), '{$path}', 'cover-picture', 1)");
            ...
            $update_user_pp_stmt = $config->link->query("UPDATE `users` SET `cover_photo_index` = {$index} WHERE `id` = {$_SESSION['user_id']}");
        }
    }
?>