...
function scroll_load_more () {
    $(document).scroll(function () {
        page = new FormData;
        page.append('userid', window.userid);
        page.append('page', window.feed_page);//$.cookie("feed_page"));
        ...
        if ((scrollHeight - scrollPosition) / scrollHeight === 0) {
            if ($.cookie('has_more_to_load') == "true") {
                $.ajax({
                    ...
                    data : page,
                    success : function (e) {
                        if (e.toLowerCase().trim() != 'no posts to display') {
                            $.cookie('feed_page', parseInt($.cookie('feed_page')) + 1);
                            window.feed_page++;
                    ...
                    $.ajax({
                        ...
                        success : function (e) {
                            $.ajax({
                                ...
                                method : "POST",
                                success: function (e) {
                                    console.log($(window.comment_input_element).parent().parent());
                                    $(window.comment_input_element).parent().parent().parent().find(".post-comment-section").prepend(e);
                                    load_comments();
                                    after_load_eaches();
                                    reposts();
                                }
